...
type UserPreset = { id: string; name: string; email: string };
type RegionOption = { id: string; name: string; flag?: string };

const DEMO_PRESETS: UserPreset[] = [
  { id: 'andrew', name: 'Andrew UI', email: 'andrew.ui@uisocial.com' },
  { id: 'admin', name: 'Voxera Admin', email: 'admin@voxera.ai' },
  { id: 'sarah', name: 'Sarah SDR', email: 'sarah@voxera.ai' },
];
...
function MfaModal({ isOpen, onVerifySuccess, onCancel, userEmail }: { isOpen: boolean; onVerifySuccess: () => void; onCancel: () => void; userEmail: string; }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-md">
      <div className="relative w-full max-w-md rounded-2xl bg-zinc-900 border border-zinc-700 p-6 md:p-8 text-left space-y-5 text-white">
        <div className="w-11 h-11 rounded-2xl bg-zinc-800 flex items-center justify-center text-zinc-300"><KeyRound className="w-6 h-6" /></div>
        <div className="flex items-center justify-between">
          <h3 className="text-xl font-medium">Security Verification</h3>
          <span className="text-[10px] font-mono uppercase tracking-wider px-2.5 py-0.5 rounded-full bg-zinc-800 text-zinc-300">MFA Mandatory</span>
        </div>
        <p className="text-xs text-zinc-400">Enter the 6-digit TOTP code for <strong>{userEmail || 'your account'}</strong>.</p>

        <form onSubmit={handleVerify} className="space-y-5">
          <div>
            <div className="flex items-center justify-between gap-2 mb-2">
              {code.map((digit, idx) => (
                <input key={idx} ref={(el) => { inputRefs.current[idx] = el; }} type="text" inputMode="numeric" maxLength={1} value={digit} onChange={(e) => handleChange(idx, e.target.value)} onKeyDown={(e) => handleKeyDown(idx, e)} className="w-12 h-12 text-center text-xl font-medium rounded-xl bg-black border border-zinc-700 text-white focus:outline-none focus:border-white" />
              ))}
            </div>
            {error && <p className="mt-2 text-xs text-red-500">{error}</p>}
          </div>

          <div className="p-3.5 rounded-xl bg-zinc-800/60 flex items-center justify-between">
            <span className="text-xs text-zinc-400">Demo Testing?</span>
            <button type="button" onClick={fillDemoCode} className="text-xs text-amber-400 hover:underline">Autofill MFA Code (842910)</button>
          </div>

          <div className="flex items-center justify-end gap-3 pt-2">
            <button type="button" onClick={onCancel} className="px-4 py-2.5 rounded-xl text-xs bg-zinc-800 hover:bg-zinc-700">Cancel</button>
            <button type="submit" disabled={isVerifying} className="px-6 py-3 rounded-xl bg-white text-zinc-950 font-bold text-xs flex items-center gap-2">
...
function ForgotPasswordModal({ isOpen, onClose, initialEmail = '' }: { isOpen: boolean; onClose: () => void; initialEmail?: string }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-md">
      <div className="relative w-full max-w-md rounded-2xl bg-zinc-900 border border-zinc-700 text-white overflow-hidden p-6 md:p-8 text-left space-y-4">
        <button onClick={onClose} className="absolute top-5 right-5 p-2 rounded-xl bg-zinc-800 text-zinc-400 hover:text-white" aria-label="Close"><X className="w-4 h-4"/></button>
        {!isSubmitted ? (
          <div>
            <div className="w-11 h-11 rounded-2xl bg-zinc-800 flex items-center justify-center mb-4"><Mail className="w-6 h-6"/></div>
            <h3 className="text-xl font-medium mb-1">Reset Work Password</h3>
            <p className="text-xs text-zinc-400 mb-6">Enter your corporate email address. A password reset link will be dispatched.</p>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label htmlFor="reset-email" className="block text-xs font-medium mb-1.5">Work Email Address</label>
                <input id="reset-email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="name@company.com" className="w-full px-4 py-3 rounded-xl bg-black border border-zinc-700 text-xs text-white" required />
                {error && <p className="mt-2 text-xs text-red-500">{error}</p>}
              </div>
              <div className="pt-2 flex items-center justify-end gap-3">
                <button type="button" onClick={onClose} className="px-4 py-2.5 rounded-xl text-xs bg-zinc-800">Cancel</button>
                <button type="submit" disabled={isLoading} className="px-6 py-3 rounded-xl bg-white text-zinc-950 font-bold text-xs">
          ...
          <div className="text-center py-4">
            <div className="w-12 h-12 rounded-full bg-emerald-950 text-emerald-400 flex items-center justify-center mx-auto mb-4"><CheckCircle2 className="w-6 h-6"/></div>
            <h3 className="text-lg font-medium mb-2">Reset Instructions Sent</h3>
            <p className="text-xs text-zinc-400 mb-6">We dispatched reset instructions to <strong>{email}</strong>. Check your corporate inbox.</p>
            <button onClick={handleReset} className="w-full py-3 rounded-xl bg-white text-zinc-950 font-bold text-xs">Return to Login</button>
          </div>
        )}
      </div>
    </div>
  ...
  return (
    <div className="w-full max-w-5xl mx-auto p-2 sm:p-4 my-auto select-none">
      ...
      <div className="bg-white/95 backdrop-blur-xl rounded-[36px] sm:rounded-[42px] p-3 sm:p-4 shadow-2xl border border-zinc-200 overflow-hidden grid grid-cols-1 lg:grid-cols-12 gap-4 lg:gap-6 items-stretch relative z-10">
        <div className="robot-hero-card lg:col-span-6 relative overflow-hidden min-h-[480px] sm:min-h-[560px] lg:min-h-[620px] flex flex-col justify-between p-6 sm:p-8 text-white bg-black group shadow-2xl rounded-[28px]" style={{ clipPath: 'url(#slantedInnerBox)' }}>
        ...
        <div className="lg:col-span-6 bg-white rounded-[28px] sm:rounded-[32px] p-6 sm:p-8 lg:p-10 flex flex-col justify-between text-zinc-900">
          <div className="flex items-center justify-between gap-2 mb-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xl font-bold tracking-tight text-zinc-900 uppercase font-mono">VOXERA AI</span>
            </div>
            <div className="flex items-center gap-2">
                    ...
                    <div className="border-t border-zinc-800 pt-2">
                      ...
                      <div className="grid grid-cols-3 gap-1 text-xs font-mono">
                        {[{ id: 'waves', label: 'Waves' }, { id: 'spotlight', label: 'Spotlight' }, { id: 'matrix', label: 'Rain' }].map((m) => (
                          <button key={m.id} type="button" onClick={() => onSelectMode && onSelectMode(m.id as AsciiMode)} className={`py-1 px-1.5 rounded-lg text-center text-[10px] transition ${asciiMode === m.id ? 'bg-amber-500 text-zinc-950 font-bold' : 'bg-zinc-800 text-zinc-400'}`}>{m.label}</button>
                        ))}
                      </div>
                    </div>
                  </div>
              ...
              <div className="relative">
                <button type="button" onClick={() => { setIsRegionOpen(!isRegionOpen); setIsAsciiControlOpen(false); }} className="px-3.5 py-1.5 rounded-full border border-zinc-200 bg-white text-xs font-medium text-zinc-700"><span>{selectedRegion.flag} {selectedRegion.name.split(' ')[0]}</span><ChevronDown className="w-3.5 h-3.5 text-zinc-400"/></button>
                {isRegionOpen && (
          ...
          <div className="my-auto py-2">
            <div className="text-center mb-6">
              <h2 className="text-3xl sm:text-4xl font-bold text-zinc-900 tracking-tight">Hi {selectedPreset.name.split(' ')[0]}</h2>
              <p className="text-xs sm:text-sm font-normal text-zinc-500 mt-1.5">Welcome to Voxera AI OS</p>
            </div>

            {/* Account Preset Switcher Pills */}
            ...
            <form onSubmit={handleSubmit} className="space-y-4 text-left">
              <div>
                <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Email" required className="w-full border border-zinc-200 rounded-xl px-4 py-3 text-zinc-900 text-sm placeholder-zinc-400 focus:ring-1 focus:ring-zinc-800 outline-none" />
              </div>
              <div>
                <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Password" required className="w-full border border-zinc-200 rounded-xl px-4 py-3 text-zinc-900 text-sm placeholder-zinc-400 focus:ring-1 focus:ring-zinc-800 outline-none" />
              </div>
              <div className="flex justify-end">
                <button type="button" onClick={() => onOpenForgotPassword(email)} className="text-xs font-medium text-zinc-600 hover:underline">Forgot password ?</button>
              </div>
...
export function LoginPage() {
  ...
  const handleLoginSubmit = async (email: string, pass: string, _mode: LoginMode, _preset?: UserPreset) => {
    ...
    <div className="min-h-screen bg-black text-slate-100 flex flex-col justify-between font-sans relative overflow-x-hidden">
      ...
      <main className="relative z-10 flex-1 flex flex-col items-center justify-center px-4 py-8 md:py-12 w-full max-w-7xl mx-auto my-auto">
        <RedesignedLoginPortal
          ...
          onOpenComplianceModal={() => alert("Voxera AI Platform - Security Brief & Compliance Document: ISO 27001, SOC 2 Type II, HIPAA & GDPR Compliant.")}
